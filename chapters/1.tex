...
\begin{proof}
    ...
    所以有
    \[\inf\bigl\{f\bigl[U_E^\delta(x_0)\bigr]\bigr\} \leqslant \inf\bigl\{m\bigl[U_E^\delta(x_0)\bigr]\bigr\} \leqslant \sup\bigl\{m\bigl[U_E^\delta(x_0)\bigr]\bigr\} \leqslant \sup\bigl\{f\bigl[U_E^\delta(x_0)\bigr]\bigr\}\]
...
\begin{proof}
    ...
    因为
    \[\inf_{a \leqslant \xi < x_0} \{m(\xi)\} = A(x_0) \geqslant m(x_0) = \inf_{a \leqslant \xi \leqslant x_0} \{m(\xi)\}\]
    是显然的，接下来尝试证明
    \[A(x_0) \leqslant m(x_0)\]

    根据 $\inf$ 的定义，我们知道对于任意的 $x_0$，我们任取满足 $a \leqslant x'' < x' < x_0$ 的实数 $x'', x'$ 都有
    \[A(x_0) = \inf_{a \leqslant \xi < x_0}\{m(\xi)\} \leqslant m(x') = \inf_{a \leqslant \xi < x'}\{f(\xi)\} \leqslant f(x'')\]

    于是我们证明了
    \[\forall a \leqslant \xi < x_0: A(x_0) \leqslant f(\xi)\]
    继而
    \[A(x_0) \leqslant \inf_{a \leqslant \xi < x_0} \{f(\xi)\} = m(x_0)\]
\end{proof}\vspace{9pt}

\textbf{752.} 设函数 $f(x)$ 在区间 $(0, +\infty)$ 上连续并有界. 求证：对于任何正实数 $T$，可求得数列 $x_n \rightarrow +\infty$ 使
...
\begin{proof}
    ...
    如果
    \[\forall n \in \mathbb{N}^*: g(x' + nT) \cdot g(x') > 0 \tag{752.1}\]
    那么 $x' \in A$，否则 $x' \in B$.

    易得 $A \cap B = \varnothing, A \cup B = \mathbb{R}^+$.

    如果 $A = \varnothing$，那么
    \[\forall x' \in B = \mathbb{R}^+: \exists x'' \geqslant x': g(x'') = 0\]

    所以可以找到一个数列 $x_n$ 使得
    \[g(x_n) = 0, \quad x_n \rightarrow \infty\]
    从而满足题意.
...
\begin{proof}
    ...
    设正实数 $T_1, T_2$ 满足
    \begin{gather*}
        \varphi(x+T_1) = \varphi(x)\\
        \psi(x+T_2) = \psi(x)
    \end{gather*}

    运用反证法，如果原命题不成立，则存在点 $x'$ 使得 $\varphi(x') \neq \psi(x')$.

    数论知识告诉我们，对于任意的正实数 $\delta$，都能找到正整数 $p,q$ 使得
    \[\lvert pT_1 - qT_2\rvert < \delta\]

    我们将这个结论转变成另一个有用的形式\footnote{其实并不完全是等价命题转换，还要额外证明一些小性质}：存在正整数列 $p_n, q_n$ 使得当 $n \rightarrow \infty$ 时
    \begin{gather*}
        p_n \rightarrow +\infty\\
        q_n \rightarrow +\infty\\
        \lvert p_nT_1 - q_nT_2\rvert \rightarrow 0
    \end{gather*}

    而根据一致连续性可知，对于数列 $a_n, b_n$，当 $n \rightarrow \infty$ 时
    \[a_n - b_n \rightarrow 0 \Rightarrow \Bigl[\varphi(a_n) - \varphi(b_n) \rightarrow 0 \wedge \psi(a_n) - \psi(b_n) \rightarrow 0\Bigr]\]
    然后我们现在来考察数列 $\varphi(x' + p_nT_1) - \psi(x' + q_nT_2)$.

    因为
    \begin{gather*}
        \lim_{n \rightarrow \infty} \varphi(x' + p_n T_1) = \varphi(x')\\
        \lim_{n \rightarrow \infty} \psi(x' + q_n T_2) = \psi(x')\\
        \lim_{n \rightarrow \infty} (x' + p_n T_1) - (x' + q_n T_2) = 0
    \end{gather*}
    所以
    \begin{gather*}
        \lim_{n \rightarrow \infty} \varphi(x' + p_nT_1) - \psi(x' + q_nT_2) = \varphi(x') - \psi(x')\\
        \lim_{n \rightarrow \infty} \varphi(x' + p_n T_1) - \varphi(x' + q_n T_2) = 0
    \end{gather*}
    所以
    \[\lim_{n \rightarrow \infty} \varphi(x' + q_n T_2) - \psi(x' + q_n T_2) = \varphi(x') - \psi(x')\]
